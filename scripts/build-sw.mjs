...
const injectManifestOptions = {
  ...
  manifestTransforms: [
    async (manifestEntries) => {
      const filteredEntries = manifestEntries.filter((e) => !e.url.includes("index.html"));
...
async function buildServiceWorker() {
  try {
    ...
    console.log(
      `Service worker built successfully! Precached ${count} files, totaling ${size} bytes.`,
    );
  } catch (error) {
