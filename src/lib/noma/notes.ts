...
export const toggleFavorite = (n: Note) => updateNote(n.id, { favorite: !n.favorite });
export const setArchived = (n: Note, archived: boolean) => updateNote(n.id, { archived });

export const trashNote = (id: string) =>
  updateNote(id, { deleted: true, deletedAt: Date.now(), pinned: false });

export const restoreNote = (id: string) => updateNote(id, { deleted: false, deletedAt: null });
...
export async function deleteFolder(id: string): Promise<void> {
  await db().transaction("rw", db().folders, db().notes, async () => {
    ...
    for (const note of notes) await db().notes.update(note.id, { folderId: null });
...
export const renameTag = (id: string, name: string) =>
